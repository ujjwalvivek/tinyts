...
export class Registry {
    ...
    private readonly removeListeners = new Map<
        number,
        Set<ComponentCallback>
    >();

    // Serialization mapping
    private readonly registeredTypes = new Map<
        string,
        new (...args: any[]) => any
    >();
    ...
    setParent(child: Entity, parent: Entity | null): void {
        ...
        if (!childrenComp) {
            childrenComp = this.addComponent(
                parent,
                Children,
                this.obtain(Children, []),
            );
        }
    ...
    serialize(): string {
        const data: {
            entities: Entity[];
            components: Record<string, Record<Entity, any>>;
        } = {
    ...
    deserialize(json: string): void {
        ...
        for (const [typeName, storeData] of Object.entries(data.components)) {
            ...
            for (const [entityStr, compData] of Object.entries(
    ...
    view(...args: any[]): Entity[] {
        ...
        const withIds = withClasses
            .map((cls) => getComponentTypeId(cls))
            .sort((a, b) => a - b);
        const withoutIds = withoutClasses
            .map((cls) => getComponentTypeId(cls))
            .sort((a, b) => a - b);
     ...
     */
    obtain<T>(componentClass: new (...args: any[]) => T, ...args: any[]): T {
        const typeId = getComponentTypeId(componentClass);
        let pool = this.componentPools.get(typeId);
        if (!pool) {
            pool = [];
